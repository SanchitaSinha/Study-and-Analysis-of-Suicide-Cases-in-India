...
#Reading Data
sc_data <- read.csv("E:/Project/Suicide Why/Final DataSet/Suicides in India (2001-2012).csv")
...
#Which gender makes more suicide attempts?
...
label <-  c( paste(gper$Gender[1],gper$percent[1],'%',sep=''), paste(gper$Gender[2],gper$percent[2],'%',sep=''))
colr<-c("dodgerblue4","goldenrod3", "mediumpurple3","orangered4",
...
#plot a pie chart
pie3D(gper$percent,labels=label,labelcex=0.9,explode=0.15,col=colr)
...
pie3D(pdata$atot,labels=pdata$Age_group, labelcex=0.9, explode=0.1,col=colr, main="Suicide and Age Group")
...
sc_type %>% ggplot(aes(x=str_sub(Type,1,15),y=ttotal,fill=Type))+geom_boxplot()+scale_fill_manual(values=colr)+theme(legend.position = "bottom",axis.text.x = element_text(angle=90))+labs(x="Educational Status",y="Count")
...
pp_type %>% ggplot(aes(x=str_sub(Type,1,20),y=ttotal,fill=Gender))+geom_bar(stat="identity",position="fill")+scale_fill_manual(values=colr)+theme(legend.position = "bottom",axis.text.x = element_text(angle=90))+labs(x="Professional Status",y="Percentage")
pp_type%>%ggplot(aes(x=Type,y=ttotal,fill=Type))+geom_bar(stat="identity")+scale_fill_manual(values=colr)+
  theme(legend.position = "none",axis.text.x = element_text(angle=90))+labs(x="Professional Status" ,y="Count")
...
ca_type%>%ggplot(aes(x=Type,y=ctot,fill=Gender))+geom_bar(stat="identity",position="dodge")+scale_color_manual(values=colr)+theme(legend.position = "bottom",axis.text.x = element_text(angle=90,vjust=0.5))+labs(x="Causes",y="Count")
...
#Analyzing the trend for the major causes for the suicides
...
sc_data%>%select(State,Year,Total) %>% group_by(State,Year)%>% summarise(total=sum(Total)) %>% ggplot(aes(x=State,y=total,fill=Year))+geom_bar(stat="identity")+
  theme(legend.position="bottom",axis.text.x=element_text(angle=90))

###############################################################

sc_data %>%filter(!State %in% c("Total (All India)","Total (States)","Total (Uts)"))%>%ggplot(aes(x=factor(Year),y=State,fill=Total))+geom_tile()+facet_wrap(~Gender)+scale_fill_gradientn(colors=brewer.pal(5,"BuPu"))+theme(axis.text.x = element_text(angle=90))+labs(x="Year")
...
options(repr.plot.width=6, repr.plot.height=4) 
sc1<-sc_data%>%filter(!State %in% c("Total (All India)","Total (States)","Total (Uts)"))%>% select(State,Year,Total) %>% group_by(State)%>% summarise(total=sum(Total)) %>% arrange(desc(total))%>%head(10)%>%  ggplot(aes(x=factor(State,level=State),y=total,color=State))+geom_point(size=1)+geom_segment(aes(xend=State,y=0,yend=total),size=2)+theme(legend.position="none",axis.text.x=element_text(angle=90))+scale_color_manual(values=colr)+geom_text(aes(label=total),vjust=0.3)+labs(x="State and Union Territories with Highest Suicide Rates")
sc2<-sc_data%>%filter(!State %in% c("Total (All India)","Total (States)","Total (Uts)"))%>% select(State,Year,Total) %>% group_by(State)%>% summarise(total=sum(Total)) %>% arrange(desc(total))%>%tail(10)%>%   ggplot(aes(x=factor(State,level=State),y=total,color=State))+geom_point(size=1)+geom_segment(aes(xend=State,y=0,yend=total),size=2)+theme(legend.position="none",axis.text.x=element_text(angle=90))+scale_color_manual(values=colr)+geom_text(aes(label=total),vjust=0.3)+labs(x="State and Union Territories with Lowest Suicide Rates")
